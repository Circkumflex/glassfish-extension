 ...
 * 
 */
public final class QueryBasedJDBCRealm extends MultipleLoginRealm {
    ...
    public static final String PARAM_USER_QUERY = "user-query";
    ...
    public synchronized void init(Properties props)
        ...
        userQuery = props.getProperty(PARAM_USER_QUERY);
        passwordQuery = props.getProperty(PARAM_PASSWORD_QUERY);
        groupQuery = props.getProperty(PARAM_GROUP_QUERY);
        ...
        if (userQuery == null) {
            String msg = sm.getString(
                    "realm.missingprop", PARAM_USER_QUERY, "JDBCRealm");
            throw new BadRealmException(msg);
        }
        if (passwordQuery == null) {
            String msg = sm.getString(
                    "realm.missingprop", PARAM_PASSWORD_QUERY, "JDBCRealm");
            throw new BadRealmException(msg);
        }
        if (groupQuery == null) {
            String msg = sm.getString(
                    "realm.missingprop", PARAM_GROUP_QUERY, "JDBCRealm");
            throw new BadRealmException(msg);
        }
    ...
    private boolean isUserValid(String user, char[] password) {
        ...
        try {
            ...
            int paramCount = passwordQuery.length() - passwordQuery.replaceAll("\\?", "").length();
            for (int i = 1; i <= paramCount; i++) {
    ...
    private String[] findGroups(String user) {
        ...
        try {
            ...
            int paramCount = groupQuery.length() - groupQuery.replaceAll("\\?", "").length();
            for (int i = 1; i <= paramCount; i++) {
                statement.setString(i, user);
            }
